...
async function listWorkoutsForUser(userId) {
    ...
    const { rows } = await query(`select w.id, w.day_number, w.title, w.focus, w.duration_minutes, w.description, w.video_url,
            coalesce(uwp.completed, false) as completed
     from workouts w
     left join user_workout_progress uwp
       on uwp.workout_id = w.id and uwp.user_id = $1
     order by w.day_number asc`, [userId]);
    const workouts = rows.map((w) => {
        let status = 'bloqueado';
        if (w.completed)
            status = 'concluido';
        else if (w.day_number <= currentDay)
            status = 'disponivel';
        return {
            ...w,
            status,
...
router.get('/workouts/today', auth, async (req, res, next) => {
    try {
       ...
       left join user_workout_progress uwp
         on uwp.workout_id = w.id and uwp.user_id = $1
       where w.day_number = $2`, [req.user.id, currentDay]);
...
router.post('/workouts/:id/complete', auth, async (req, res, next) => {
    try {
        ...
        await query(`insert into user_workout_progress (id, user_id, workout_id, completed, completed_at)
       values (gen_random_uuid(), $1, $2, true, now())
       on conflict (user_id, workout_id)
       do update set completed = true, completed_at = now()`, [req.user.id, workout.id]);
