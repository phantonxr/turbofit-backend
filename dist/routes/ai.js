...
async function chat(req, res, next, assistantType) {
    try {
        ...
        const { rows: history } = await query(`select role, content
       from ai_messages
       where user_id = $1 and assistant_type = $2
       order by created_at desc
       limit 20`, [req.user.id, assistantType]);
        const system = assistantType === 'lia-fit'
    ...
    - Se houver sinais de condição médica, gravidez/aleitamento com restrições, ou transtorno alimentar: recomende procurar um profissional de saúde.`;
        ...
        await query(`insert into ai_messages (id, user_id, assistant_type, role, content)
       values (gen_random_uuid(), $1, $2, 'user', $3)`, [req.user.id, assistantType, message]);
        ...
        await query(`insert into ai_messages (id, user_id, assistant_type, role, content)
       values (gen_random_uuid(), $1, $2, 'assistant', $3)`, [req.user.id, assistantType, answer]);
...
router.get('/ai/lia-fit/history', auth, requireSubscription, async (req, res, next) => {
    try {
        const { rows } = await query(`select role, content
       from ai_messages
