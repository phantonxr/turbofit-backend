...
router.get('/habits/today', auth, async (req, res, next) => {
    try {
        ...
        const { rows } = await query(`select h.id, h.day_number, h.title, h.description,
       ...
       order by h.created_at asc`, [req.user.id, day]);
...
router.get('/habits', auth, async (req, res, next) => {
    try {
       ...
       left join user_habit_progress uhp
         on uhp.habit_id = h.id and uhp.user_id = $1
       order by h.day_number asc, h.created_at asc`, [req.user.id]);
...
router.post('/habits/:id/complete', auth, async (req, res, next) => {
    try {
        ...
        await query(`insert into user_habit_progress (id, user_id, habit_id, status, fail_reason, completed_at)
       values (gen_random_uuid(), $1, $2, 'completed', null, now())
       on conflict (user_id, habit_id)
       do update set status = 'completed', fail_reason = null, completed_at = now()`, [req.user.id, habit.id]);
...
router.post('/habits/:id/fail', auth, async (req, res, next) => {
    try {
       ...
       on conflict (user_id, habit_id)
       do update set status = 'failed', fail_reason = $3, completed_at = now()`, [req.user.id, habit.id, reason]);
        return res.json({ ok: true });
    }
    catch (err) {
