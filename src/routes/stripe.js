...
function getStripe()
{
  if (!process.env.STRIPE_SECRET_KEY) throw new Error('STRIPE_SECRET_KEY não definido');
...
router.post('/stripe/create-checkout-session', auth, async (req, res, next) =>
{
  try
  {
    ...
    if (!priceId) return res.status(500).json({ error: 'Preço do Stripe não configurado no backend' });
    ...
    if (!customerId)
    {
      const customer = await stripe.customers.create({
        email: user.email,
        name: user.name,
        metadata: { app: 'turbofit', vendor: 'Albuca', user_id: user.id },
      });
...
router.post('/stripe/webhook', async (req, res) =>
{
  ...
  try
  {
    ...
    if (!sig || Array.isArray(sig)) return res.status(400).send('Assinatura Stripe ausente');
  ...
  try
  {
    ...
    // ignora eventos de outros projetos/sistemas no mesmo account Stripe
    function isTurboFit(meta)
    {
      return meta && meta.app === 'turbofit';
    }

    if (event.type === 'checkout.session.completed')
    {
      if (!isTurboFit(obj.metadata)) return res.json({ received: true });
    ...
    {
      if (!isTurboFit(obj.metadata)) return res.json({ received: true });
      const nextStatus = toSubscriptionStatus(obj.status);
      await query('update users set subscription_status = $1 where stripe_customer_id = $2', [nextStatus, obj.customer]);
    }

    if (event.type === 'customer.subscription.deleted')
    {
      if (!isTurboFit(obj.metadata)) return res.json({ received: true });
      await query("update users set subscription_status = 'inactive' where stripe_customer_id = $1", [obj.customer]);
    }

    if (event.type === 'invoice.payment_succeeded')
    {
      // invoice não carrega metadata de subscription — filtra pela presença do customer no nosso banco
      const customer = obj.customer;
      if (customer)
      {
        await query("update users set subscription_status = 'active' where stripe_customer_id = $1", [customer]);
  ...
  try
  {
    ...
    if (!customerId)
    {
      const customer = await stripe.customers.create({
        email: user.email,
        name: user.name,
        metadata: { app: 'turbofit', vendor: 'Phanton', user_id: user.id },
      });
      customerId = customer.id;
      await query('update users set stripe_customer_id = $1 where id = $2', [customerId, user.id]);
    }
