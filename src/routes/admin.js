...
router.patch('/admin/users/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `update users set ${sets} where id = $${values.length} returning id, name, email, subscription_status, role`,
      values
    );
...
router.get('/admin/users/:id/progress', async (req, res, next) => {
  try {
    const { rows: workouts } = await query(
      `select w.id, w.day_number, w.title, uwp.completed, uwp.completed_at
       from workouts w
    ...
    const { rows: habits } = await query(
      `select h.id, h.day_number, h.title, uhp.status, uhp.fail_reason, uhp.completed_at
       from habits h
...
router.post('/admin/workouts', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `insert into workouts (id, day_number, title, focus, duration_minutes, description, video_url)
       values (gen_random_uuid(), $1, $2, $3, $4, $5, $6)
       returning *`,
      [data.day_number, data.title, data.focus, data.duration_minutes, data.description ?? null, data.video_url ?? null]
    );
...
router.patch('/admin/workouts/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `update workouts set ${sets} where id = $${values.length} returning *`,
      values
    );
    if (!rows.length) return res.status(404).json({ error: 'Treino não encontrado' });
...
router.post('/admin/habits', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `insert into habits (id, day_number, title, description)
       values (gen_random_uuid(), $1, $2, $3)
       returning *`,
      [data.day_number, data.title, data.description ?? null]
    );
...
router.patch('/admin/habits/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `update habits set ${sets} where id = $${values.length} returning *`,
      values
    );
    if (!rows.length) return res.status(404).json({ error: 'Hábito não encontrado' });
...
router.delete('/admin/habits/:id', async (req, res, next) => {
  try {
    ...
    if (!rowCount) return res.status(404).json({ error: 'Hábito não encontrado' });
    return res.json({ ok: true });
  } catch (err) {
    return next(err);
  }
...
router.post('/admin/videos', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `insert into videos (id, title, category, duration_minutes, video_url, thumbnail_url, description)
       values (gen_random_uuid(), $1, $2, $3, $4, $5, $6)
       returning *`,
      [data.title, data.category, data.duration_minutes, data.video_url ?? null, data.thumbnail_url ?? null, data.description ?? null]
    );
...
router.patch('/admin/videos/:id', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `update videos set ${sets} where id = $${values.length} returning *`,
      values
    );
