...
router.post('/register', async (req, res, next) => {
  try {
    ...
    const { rows } = await query(
      `insert into users (id, name, email, password_hash, main_goal, pain_notes)
       values (gen_random_uuid(), $1, $2, $3, $4, $5)
       returning id, name, email, main_goal, pain_notes, subscription_status, role, created_at`,
      [data.name, data.email.toLowerCase(), passwordHash, data.main_goal, data.pain_notes ?? null]
    );
